...
| **PPO** | **95% (Strong Refusals)** | < 10% (Forgetting) | Safe but hallucinates procedural details. |
| **GRPO** | 90% (Refusals) | < 10% (Forgetting) | Similar to PPO but faster training. |
...
*   **PPO Model:** *"[ACTION REQUIRED: Copy the PPO result for this prompt from your CSV]"*
...
*   **Base GPT-2:** *"[ACTION REQUIRED: Copy the toxic Base result from your CSV]"*
*   **DPO Model:** *"[ACTION REQUIRED: Copy the safe DPO refusal from your CSV]"*
...
*   **GRPO:** **Winner in Efficiency.** By estimating advantages using group averages (Group Size = 4) instead of a learned Value function, GRPO removed the need for the Critic model. This reduced memory usage and increased training speed by approximately 20%.

**[ACTION REQUIRED: Paste a screenshot of your PPO or DPO Loss curve from WandB here]**

---

## Conclusion

This assignment demonstrated the power and the pitfalls of RLHF. We successfully transformed a toxic GPT-2 model into a safe, aligned assistant using SFT, PPO, GRPO, and DPO. However, the constraints of the 124M parameter architecture led to severe **Alignment Tax**, where the model sacrificed its intelligence to ensure safety.

**Key Takeaway:** For effective RLHF that preserves capabilities, a model likely needs significantly more parameters (>1B) to maintain separate subspaces for factual knowledge and alignment behaviors. Among the methods tested, **GRPO** offered the best balance of implementation simplicity and computational efficiency for this scale of experimentation.
